...
void NRPyEllipticET_PunctureInitialData_Initialize_ADMBase( const cGH *restrict cctkGH,
  ...
  paramstruct params;
#include "set_Cparameters_from_parfile_paramstruct.h"
  ...
  for(int k=kmin;k<kmax;k++) {
    for(int j=jmin;j<jmax;j++) {
      for(int i=imin;i<imax;i++) {
        ...
        CCTK_REAL xx_star[3];
        get_xx_from_xyz(domain_size,sinh_width,foci_position,xCart,xx_star);
...
#pragma omp parallel for
  ...
  NRPyEllipticET_interpolate_solution_to_ADMBase(num_pts,
                                                 input_array_dims,
                                                 origin,deltas,
                                                 interp_coords,
                                                 input_array,
...
#pragma omp parallel for
  ...
  compute_ADM_Cartesian_quantities_from_uu(cctkGH,orbital_plane,&params,
