...
#include "free_parameters.h"
  ...
  const REAL wavespeed_at_OB = griddata.gridfuncs.auxevol_gfs[IDX4S(WAVESPEEDGF, 
                                                              Nxx_plus_2NGHOSTS0 - NGHOSTS - 1,
                                                              NGHOSTS, 
                                                              Nxx_plus_2NGHOSTS2/2)];
                                                                
  for (int ii = 0; ii < NUM_EVOL_GFS; ii++){
    gridfunctions_wavespeed[ii] = wavespeed_at_OB;
    printf("gridfunctions_wavespeed[%d] = %.16e\n", ii, gridfunctions_wavespeed[ii]);
  }
  
  // Step ?: Create output folders
  const int output_folder          = mkdir("./output_NRPyElliptic", 0777);
  const int uu_output_folder       = mkdir("./output_NRPyElliptic/uu_1D", 0777);
  const int residual_output_folder = mkdir("./output_NRPyElliptic/residual_1D", 0777);
  ...
  for(int n=0;n<=N_final;n++) { // Main loop to progress forward in time.
    ...
    // Step 2.b: Output gridfunctions
    if (n%output_every_N==0){
      // Step 2.b.i: uu along z-axis
      char uu_1D_file_string[100];
      sprintf(uu_1D_file_string, "output_NRPyElliptic/uu_1D/uu_1D_%08d.dat", n);
    ...
    /* 
    // Step 2.b.ii: residual along z-axis
    char residual_1D_file_string[100];
    sprintf(residual_1D_file_string, "output_NRPyElliptic/residual_1D/residual_1D_%08d.dat", n);
    FILE *residual_1D_file = fopen(residual_1D_file_string, "w");
    gridfunction_z_axis(&griddata.params, UUGF, griddata.xx, griddata.gridfuncs.diagnostic_output_gfs, residual_1D_file);
    fclose(residual_1D_file);
    */  
    // Step 2.c: compute and output L2-norm of residual
    ...
    fprintf(residual_l2_norm_file, "%10.4e %.17e\n", griddata.params.time, l2_norm_residual);
